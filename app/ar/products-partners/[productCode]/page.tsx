...
return <main dir="rtl"><SectionShell><nav className="mb-4 text-sm"><Link href="/ar/products-partners" className="hover:underline">العودة إلى المنتجات</Link></nav><section className="rounded-2xl border border-slate-700 bg-gradient-to-br from-slate-950 via-navy-900 to-slate-900 p-4 text-white md:p-8"><div className="grid gap-6 lg:grid-cols-2"><div className="relative aspect-[4/3] overflow-hidden rounded-xl border border-white/15 bg-white/5">{productImageSrc ? <Image src={productImageSrc} alt={productImageAlt} fill className="object-contain p-4" /> : null}</div><div><p className="text-xs text-orange-300">{localizeCategory(product.category)}</p><h1 className="mt-2 text-2xl font-bold">{product.name}</h1><p className="mt-2 text-sm">العلامة التجارية: {product.brand}</p><p className="mt-1 text-sm">التصنيف: {localizeCategory(product.category)}</p><p className="mt-2 rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-sm">{product.priceNote?.trim() ? `${arProductsMessages.priceRefLabel} ${product.priceNote.trim()}` : 'السعر عند الطلب'}</p><p className="mt-2 rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-sm">يتم تأكيد السعر والتوفر أثناء مراجعة طلب عرض السعر</p><ProductDetailActions product={product} intelligenceHref={intelligenceSlug ? `/ar/products-partners/intelligence/${intelligenceSlug}` : undefined} productsHref="/ar/products-partners" labels={{ addToRFQ: 'أضف لطلب عرض السعر', technicalNotes: 'ملاحظات فنية', backToProducts: 'العودة إلى المنتجات', addedToRFQ: 'تمت الإضافة' }} /></div></div></section><section className="mt-4 grid gap-4 md:grid-cols-2"><article className="rounded-xl border bg-white p-5"><h2 className="font-semibold">المواصفات</h2><p className="mt-2 text-sm">{product.shortSpecs}</p></article><article className="rounded-xl border bg-white p-5"><h2 className="font-semibold">الاستخدامات</h2><p className="mt-2 text-sm">{product.useCase}</p></article></section><section className="mt-4 rounded-xl border bg-white p-5"><h2 className="font-semibold">ملاحظات فنية</h2><p className="mt-2 text-sm">{product.technicalNotes?.trim() || 'يتم تأكيد التفاصيل الفنية أثناء مراجعة طلب عرض السعر.'}</p></section><section className="mt-4 rounded-xl border bg-white p-5"><h2 className="font-semibold">غالبًا يُطلب معه</h2><div className="mt-3 flex flex-wrap gap-2">{oftenQuotedWith.map((item) => <Link key={item} href="/ar/products-partners" className="rounded-full border border-slate-300 px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50">{item}</Link>)}</div></section></SectionShell></main> }
