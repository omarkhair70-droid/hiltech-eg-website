...
function ProductCard({
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md md:p-5">
      <div className="mb-4 h-1 w-12 rounded-full bg-orange-400" />
      <p className="mb-2 inline-flex w-fit rounded-full border border-orange-100 bg-orange-50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-orange-700 md:text-[10px]">
        {category}
      </p>
      ...
      <div className="mt-auto pt-3">
        <div className="flex flex-wrap gap-2">
          <Link className="btn-primary px-4 py-2.5 text-sm" href="/contact">
            Request Availability
          </Link>
          <a
            className="inline-flex items-center rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:bg-slate-50"
            href="https://wa.me/201555357807"
          >
            WhatsApp
          </a>
        </div>
      ...
      <div className="container">
        ...
        <section className="mt-6 rounded-xl border border-slate-200 bg-white p-3 md:sticky md:top-16 md:z-20 md:mt-8 md:bg-white/95 md:p-4 md:backdrop-blur">
          ...
          <div className="mt-2 flex gap-2 overflow-x-auto pb-1 md:flex-wrap md:overflow-visible">
            <a
              className="shrink-0 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700 shadow-sm md:px-4 md:py-1.5 md:text-sm"
              href="#all"
            >
              All
            </a>
            {productCategories.map((category) => (
              <a
                key={category}
                className="shrink-0 rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-medium text-slate-700 transition hover:border-slate-400 hover:bg-slate-50 md:px-4 md:py-1.5 md:text-sm"
        ...
        <section id="all" className="mt-10 space-y-12">
          {productCategories.map((category) => {
            ...
            return (
              <div key={category} id={categoryId} className="scroll-mt-24 md:scroll-mt-20">
