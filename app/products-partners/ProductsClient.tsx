...
export default function ProductsClient({ initialProducts = staticProducts, locale = 'en', messages, rfqHref = '/rfq', productsHref = '/products-partners' }: { initialProducts?: ProductItem[]; locale?: 'en'|'ar'; messages?: any; rfqHref?: string; productsHref?: string }) {
  ...
  const displayBundles = projectBundles.map((bundle) => ({
    ...
    description: isArabic ? bundle.descriptionAr : bundle.descriptionEn,
  ...
  const addBundleToRFQ = (bundleId: string) => {
    const bundle = projectBundles.find((b) => b.id === bundleId);
    if (!bundle) return;
    ...
    setItems((prev) => {
      ...
      for (const productId of bundle.productIds) {
        const found = nextItems.find((entry) => entry.id === productId);
        if (found) {
          nextItems = nextItems.map((entry) =>
            entry.id === productId ? { ...entry, quantity: normalizeRFQQuantity(entry.quantity + 1) } : entry
          );
        } else {
          const product = initialProducts.find((p) => p.id === productId);
          if (product) {
            nextItems.push(toRFQItem(product));
      ...
      trackEvent('bundle_add_to_rfq', {
        bundle_id: bundleId,
        bundle_title: isArabic ? bundle.titleAr : bundle.titleEn,
        product_count: bundle.productIds.length,
        new_products_added: addedProductIds.length,
        basket_count_after: nextItems.length,
      });
