...
function ProductVisual({ item }: { item: ProductItem }) {
  ...
  if (!visual || broken) {
    return (
      <div className="flex h-36 items-center justify-center rounded-xl border border-slate-200 bg-gradient-to-br from-slate-100 to-slate-50 text-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Illustrative visual</p>
          <p className="mt-1 px-4 text-sm font-medium text-slate-700">{item.name}</p>
        </div>
      </div>
    );
  }
