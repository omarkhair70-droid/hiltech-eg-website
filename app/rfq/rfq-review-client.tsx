    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border p-4"><p className="font-semibold">Project details</p><div className="mt-2 space-y-2">{projectFields.map(([key, label]) => { const isRequired = key === 'fullName' || key === 'phoneNumber' || key === 'emailAddress'; const error = key === 'fullName' ? errors.fullName : key === 'phoneNumber' ? errors.phoneNumber : key === 'emailAddress' ? errors.emailAddress : ''; const ref = key === 'fullName' ? fullNameRef : key === 'phoneNumber' ? phoneRef : key === 'emailAddress' ? emailRef : undefined; return <div key={key}><input ref={ref as any} className={`w-full rounded border px-3 py-2 text-sm ${error ? 'border-red-500' : ''}`} placeholder={`${label}${isRequired ? ' *' : ''}`} value={project[key] || ''} onChange={(e) => { setProject((p) => ({ ...p, [key]: e.target.value })); if (error) setErrors((prev) => ({ ...prev, [key]: '' })); }} />{error ? <p className="mt-1 text-xs text-red-700">{error}</p> : null}</div>; })}<textarea className="w-full rounded border px-3 py-2 text-sm" rows={3} placeholder="Project Notes" value={project.projectNotes || ''} onChange={(e) => setProject((p) => ({ ...p, projectNotes: e.target.value }))} /></div></div>
          ...
          {submitState.status === 'success' ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900"><p className="font-semibold">Request received successfully</p><p className="mt-1">Your RFQ reference: <span className="font-semibold">{submitState.requestCode}</span></p><p className="mt-1 text-emerald-800">Save this reference to track your request.</p><button className="mt-2 inline-flex rounded border border-emerald-300 bg-white px-3 py-1 text-xs font-medium" onClick={async () => { try { await navigator.clipboard.writeText(submitState.requestCode); setCopyReferenceState('copied'); setTimeout(() => setCopyReferenceState('idle'), 1500); } catch {} }}>Copy RFQ Reference</button>{copyReferenceState === 'copied' ? <p className="mt-1 text-xs">Copied</p> : null}</div> : null}
