...
export default async function RFQDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="section"><div className="container space-y-5"><a href="/admin/rfq" className="text-sm font-semibold text-navy-900 underline">← Back to list</a>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 text-sm"><h2 className="mb-2 font-semibold">RFQ Metadata</h2><ul className="space-y-1"><li><b>Status:</b> {rfq.status}</li><li><b>Urgency:</b> {rfq.urgency || '—'}</li><li><b>Created:</b> {new Date(rfq.created_at).toLocaleString()}</li><li><b>Last status change:</b> {rfq.last_status_changed_at ? new Date(rfq.last_status_changed_at).toLocaleString() : '—'}</li></ul><h3 className="mb-1 mt-4 font-semibold">Notification audit</h3><ul className="space-y-1"><li><b>Attempted at:</b> {rfq.notification_attempted_at ? new Date(rfq.notification_attempted_at).toLocaleString() : '—'}</li><li><b>Sent at:</b> {rfq.notification_sent_at ? new Date(rfq.notification_sent_at).toLocaleString() : '—'}</li><li><b>Provider:</b> {rfq.notification_provider || '—'}</li><li><b>Message ID:</b> {rfq.notification_message_id || '—'}</li><li><b>Error:</b> {rfq.notification_error || '—'}</li></ul></section>
    </div>
