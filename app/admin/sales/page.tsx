...
export default async function AdminSalesDashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <main className='section'><div className='container space-y-6'>
    <div className='flex flex-wrap items-center justify-between gap-3'><div><h1 className='text-2xl font-semibold'>Sales Dashboard</h1><p className='text-sm text-slate-600'>Revenue and pipeline analytics from Supabase operational RFQ and quotation data.</p></div><div className='flex gap-2'>{[7,30,90,365].map((r)=><Link key={r} href={`/admin/sales?range=${r}`} className={`rounded border px-3 py-2 text-sm ${range===r?'bg-navy-900 text-white':'border-slate-300'}`}>{r}d</Link>)}</div></div>
