...
export function Hero() {
  return <SectionShell className="bg-gradient-to-br from-navy-900 via-navy-900 to-slate-900 text-white"><div className="grid gap-8 lg:grid-cols-2 lg:items-center"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-orange-300">Enterprise Connectivity Infrastructure</p><h1 className="mt-3 text-3xl font-bold leading-[1.12] sm:text-4xl md:text-5xl">{site.positioning}</h1><p className="mt-4 max-w-2xl text-sm text-slate-100 sm:text-base">Structured cabling, fiber, data room, CCTV, and testing support for business facilities across Egypt.</p><p className="mt-3 max-w-2xl text-sm text-slate-100 sm:text-base" dir="rtl">بنية تحتية شبكية موثوقة للمشروعات والمنشآت التجارية في مصر.</p><div className="mt-6 flex flex-col gap-3 md:mt-8 sm:flex-row sm:flex-wrap"><CTAButton href="/products-partners" className="w-full justify-center sm:w-auto">Browse Products</CTAButton><CTAButton href="/solutions" variant="secondary" className="w-full justify-center sm:w-auto">Explore Solutions</CTAButton><CTAButton href="/track" variant="secondary" className="w-full justify-center border-orange-300/60 text-orange-100 hover:bg-white/10 sm:w-auto">Track RFQ</CTAButton></div></div><div className="relative rounded-2xl border border-slate-700/90 bg-navy-800/70 p-5 shadow-xl sm:p-6"><div className="mb-4 hidden h-1 w-16 rounded bg-orange-500 sm:block" /><p className="text-xs font-semibold uppercase tracking-wider text-orange-300">Project Supply Paths</p><ul className="mt-4 space-y-3 text-sm text-slate-100"><li>• <span className="font-semibold text-white">Browse Products</span> — Cabling, fiber, racks, CCTV, and testing components.</li><li>• <span className="font-semibold text-white">Build RFQ</span> — Add required items and quantities.</li><li>• <span className="font-semibold text-white">View Field Work</span> — See installation and testing references.</li><li>• <span className="font-semibold text-white">Contact HILTECH</span> — Coordinate project requirements.</li></ul></div></div></SectionShell>;
}
