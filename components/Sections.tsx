...
export function Hero() {
  return <SectionShell className="bg-navy-900 text-white"><div className="grid gap-8 lg:grid-cols-2 lg:items-center"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-orange-300">Enterprise Connectivity Infrastructure</p><h1 className="mt-3 text-3xl font-bold leading-[1.15] sm:text-4xl md:text-5xl">{site.positioning}</h1><p className="mt-4 text-sm text-slate-100 sm:text-base md:mt-5 md:text-lg">HILTECH supports infrastructure installation, testing-oriented delivery, and project supply coordination for business facilities across Egypt.</p><div className="mt-5 flex flex-col gap-2.5 sm:flex-row sm:flex-wrap md:mt-8"><CTAButton href="/contact">Request Project Quote</CTAButton><CTAButton href="/solutions" variant="secondary">Explore Solutions</CTAButton></div></div><div className="rounded-2xl border border-slate-700/80 bg-navy-800/60 p-5 sm:p-6"><p className="text-xs font-semibold uppercase tracking-wider text-orange-300">Executive Overview</p><ul className="mt-4 space-y-3 text-sm text-slate-100"><li>• Infrastructure-first approach for reliable connectivity.</li><li>• Solution pathways for cabling, fiber, data rooms, CCTV, and testing.</li><li>• Product-to-RFQ workflow for faster procurement coordination.</li></ul></div></div></SectionShell>;
}
...
export function ProductsRFQPreview() {
  return <SectionShell><SectionHeader title="Products & RFQ Workflow" description="Browse products, build your RFQ basket, and send a structured request for quotation." /><div className="mt-6 grid gap-4 md:grid-cols-3">{['Browse products', 'Review RFQ basket', 'Send via WhatsApp'].map((step, index) => <PremiumCard key={step}><p className="text-xs font-semibold uppercase tracking-wide text-orange-600">Step {index + 1}</p><p className="mt-2 font-semibold text-slate-900">{step}</p></PremiumCard>)}</div><div className="mt-6 flex flex-col gap-2.5 sm:flex-row sm:flex-wrap"><CTAButton href="/products-partners" variant="secondary">Browse Products</CTAButton><CTAButton href="/rfq" variant="link">Review RFQ Basket</CTAButton></div></SectionShell>;
}

export function TrustPreview() {
  return <SectionShell compact><div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-5"><p className="text-xs font-semibold uppercase tracking-wide text-orange-600">Assurance</p><ul className="mt-2 grid gap-1.5 text-sm text-slate-700 md:grid-cols-3">{assurancePoints.map((point) => <li key={point}>• {point}</li>)}</ul></div></SectionShell>;
}
...
export function FinalCTA() {
  return <SectionShell className="bg-navy-800 text-white"><div className="grid gap-5 lg:grid-cols-[1.25fr_0.75fr] lg:items-center"><div><SectionHeader title="Ready to Move Forward?" description="Use detailed pages for solution scope, products, and RFQ preparation — then send your request." className="text-white [&>h2]:text-white [&>p]:text-slate-100" /><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><CTAButton href="/contact">Request Project Quote</CTAButton><CTAButton href="/rfq" variant="secondary">Review RFQ Basket</CTAButton></div></div><PremiumCard className="border-slate-700 bg-navy-900/50 text-slate-100"><p className="text-sm leading-relaxed">Need quick references?</p><p className="mt-2 text-sm"><Link href="/solutions" className="underline underline-offset-2">Explore Solutions</Link> · <Link href="/products-partners" className="underline underline-offset-2">Browse Products</Link> · <Link href="/resources" className="underline underline-offset-2">View Resources</Link></p></PremiumCard></div></SectionShell>;
}
