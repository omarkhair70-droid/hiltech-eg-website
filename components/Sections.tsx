    ...
    <SectionShell className="bg-gradient-to-br from-navy-900 via-navy-900 to-slate-900 py-8 text-white sm:py-12 md:py-20">
      <div className="grid items-center gap-5 lg:grid-cols-[1.2fr_1fr] lg:gap-8">
        <div className="max-w-3xl">
          <p className="public-eyebrow text-orange-300 sm:text-sm">NETWORK INFRASTRUCTURE IN EGYPT</p>
      ...
      <div className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm sm:p-4">
        <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {trustStripItems.map((item) => (
            <li key={item} className="public-marker justify-center text-center sm:text-sm">{item}</li>
          ))}
        </ul>
      </div>
    </SectionShell>
...
export function FieldWorkPreview() {
  const previews = [
    { title: 'Fiber / ODF', description: 'Scope proof: routing context, ODF organization, and handover readiness.', image: '/fiber-splicing-workbench.jpg' },
    { title: 'Rack Preparation', description: 'Scope proof: rack layout, patching discipline, and maintainable routing.', image: '/rack-front-cabling.jpg' },
    { title: 'Structured Cabling', description: 'Scope proof: organized pathways and endpoint connectivity preparation.', image: '/copper-cable-tray.jpg' },
    { title: 'Testing & Handover', description: 'Scope proof: field validation before operational handover.', image: '/testing-otdr-device.jpg' },
  ];
  return <SectionShell className="bg-slate-50"><SectionHeader eyebrow="Field Work" title="Field Proof Prepared for Reliable Handover" description="A proof snapshot of delivery scope across fiber, racks, structured cabling, and testing workflows." /><p className="mt-2 text-sm text-slate-600">Approved real HILTECH field visuals presented with practical delivery context.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 md:gap-4">{previews.map((item)=><PremiumCard key={item.title} className="overflow-hidden bg-white p-0"><div className="relative aspect-[16/10] w-full"><Image src={item.image} alt={item.title} fill className="object-cover" sizes="(max-width: 640px) 100vw, 50vw" /></div><div className="p-3.5"><h3 className="font-semibold text-slate-900">{item.title}</h3><p className="mt-1 text-sm text-slate-700">{item.description}</p></div></PremiumCard>)}</div><div className="mt-5 md:mt-6"><CTAButton href="/work" variant="secondary" className="w-full justify-center sm:w-auto">View Real Field Work</CTAButton></div></SectionShell>;
}

export function RFQProcessSection() { return <SectionShell compact><SectionHeader eyebrow="RFQ Process" title="Clear request flow from scope to quote" description="Keep technical scope, item needs, and contact details in one request." /><div className="mt-5 grid gap-2.5 md:grid-cols-3 md:gap-4">{['Define network scope','Add required product references','Submit RFQ and track response'].map((step,index)=><PremiumCard key={step} className="bg-slate-50 p-4"><p className="public-eyebrow text-orange-600">Step {index+1}</p><p className="mt-1.5 font-semibold text-slate-900">{step}</p></PremiumCard>)}</div><div className="mt-5 flex flex-col gap-2.5 sm:flex-row"><CTAButton href="/rfq" className="w-full justify-center sm:w-auto">Request Project Quote</CTAButton><CTAButton href="/track" variant="secondary" className="w-full justify-center sm:w-auto">Track Existing RFQ</CTAButton></div></SectionShell>; }
