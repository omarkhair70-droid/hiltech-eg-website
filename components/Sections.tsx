...
export function ProductCategoriesPreview() { const cats=[['Fiber Optics','Fiber cables, ODFs, connectors, and accessories for backbone projects.'],['Structured Cabling','CAT6/CAT6A infrastructure components for office and facility networks.'],['Racks & Cabinets','Cabinets, rack accessories, cable management, and power preparation.'],['Patch Panels & Connectivity','Patch panels, keystones, couplers, and connectivity accessories.'],['CCTV Infrastructure','Camera cabling, network accessories, and control-room readiness items.'],['Testing Tools','Field testing references for fiber and copper validation workflows.'],['Power & Accessories','PDU, UPS references, and structured power accessories for racks.'],['Project Supply','Project-based product supply across coordinated network requirements.']]; return <SectionShell compact><SectionHeader title="Product Categories" description="Category-level references to help prepare technical scope before RFQ." /><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{cats.map(([title,description])=><PremiumCard key={title} className="bg-white p-4"><h3 className="text-sm font-semibold text-slate-900">{title}</h3><p className="mt-1 text-sm text-slate-700">{description}</p><Link href="/products-partners" className="mt-2 inline-block text-sm font-semibold text-orange-700 underline-offset-4 hover:underline">View Category</Link></PremiumCard>)}</div></SectionShell>; }
