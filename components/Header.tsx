    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/90 bg-white/95 shadow-[0_4px_14px_rgba(15,23,42,0.05)] backdrop-blur-md">
      <div className="container flex h-14 items-center justify-between gap-2 md:h-16 md:gap-4">
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <Link href={localizeHref('/rfq')} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:bg-slate-50">{isArabic ? `${arNavigation.rfqBasket} (${rfqCount})` : `Basket (${rfqCount})`}</Link>
          <Link href={localizeHref('/rfq')} className="inline-flex items-center rounded-md bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-orange-600">{isArabic ? 'اطلب عرض سعر' : 'Request Quote'}</Link>
        </div>
      ...
      {open ? <div id="mobile-nav" className="border-t border-slate-200 bg-white md:hidden"><div className="container py-4 pb-28"><div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="space-y-5"><section><p className="public-eyebrow px-2 text-slate-500">{isArabic ? 'الرئيسية' : 'Main'}</p><div className="mt-2 grid gap-1">{desktopNav.map(([label, href, arLabel]) => <Link key={href} href={localizeHref(href)} translate="no" onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-navy-900">{isArabic ? arLabel : label}</Link>)}</div></section><section><p className="public-eyebrow px-2 text-slate-500">{isArabic ? 'أدوات طلب العرض' : 'RFQ Tools'}</p><div className="mt-2 grid gap-2"><Link href={localizeHref('/rfq')} className="inline-flex w-full justify-center rounded-md bg-orange-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-orange-600" onClick={() => setOpen(false)}>{isArabic ? 'اطلب عرض سعر' : 'Request Quote'}</Link><Link href={localizeHref('/rfq')} className="inline-flex w-full justify-center rounded-md border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700" onClick={() => setOpen(false)}>{isArabic ? `${arNavigation.rfqBasket} (${rfqCount})` : `Basket (${rfqCount})`}</Link><Link href={localizeHref('/track')} className="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-navy-900" onClick={() => setOpen(false)}>{isArabic ? arNavigation.trackRfq : 'Track RFQ'}</Link><LanguageSwitcher className="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-navy-900" /></div></section></div></div></div></div> : null}
    </header>
  );
}
